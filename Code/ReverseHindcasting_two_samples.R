...
#observe.
obs.t1<-rep(1:sampling.time,times=round(btv.2008.interpolated[1:sampling.time,"y"]))
obs.t2<-sampling.time
infection.times<-pmin(obs.t1-rexp(length(obs.t1),rate=1/testdelay.mean),sampling.time)
...
ll.reverse.hindcasting.gen<-function(ab.values.t1,      #observed antibody test values
  ...
  ##and then calculates the likelihood of data given the parameter values P(data|params)
    ...
    function(pars){
      ...
      delay.t2<-Sampling.t2-(Infection.times.est)
      ...
      igCurve.current<-igCurve(X.star=(curve.pars["curve.pars.X.star.log"]),
                               ...
                               D=(curve.pars["curve.pars.D.log"]))
      bactLoad.current<-bactLoad(D=(curve.pars["curve.pars.D.log"]))
    ...
    post.ll<-sdlog.prior(Na.sdlog)+
      ...
      sum(ll.obs.delay(delay.t1,delay.rate))+
