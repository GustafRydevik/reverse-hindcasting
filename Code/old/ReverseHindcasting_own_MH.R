...
ll.reverse.hindcasting.gen<-function(ab.values,    #observed antibody test values
 ...
 function(Ab.curve.pars,
  ...
  #Calculating the estimated mean, based on the current parameter values of the curve
   loess.ab<-loess(ab.values~Infection.times,weights=1/ab.values^2)
   loess.na<-loess(na.values~Infection.times,weights=1/ab.values^2)
...
reverse.hindcast.jump<-function(params){

                                new.ab.pars<-rnorm(length(params$Ab.curve.pars),
                                                      mean=params$Ab.curve.pars,2)
                                new.na.pars<-rnorm(length(params$Na.curve.pars),
                                                   mean=params$Na.curve.pars,2)
                                new.ab.lognorm.theta<-rnorm(1,params$Ab.lognorm.theta,2)
                                new.na.lognorm.theta<-rnorm(1,params$Na.lognorm.theta,2)
                                new.infection.times<-rnorm(length(infection.times),params$Infection.times,2)
