...
reverse.hindcast.init<-function(){
  curve.pars<-c(log(runif(2)))#,log(runif(1,1,30))) ##Or change this to something more reasonable later...
  names(curve.pars)<-c("X.star.log","S.log")#,"a.log","D.log")
...
prev.posterior.init<-function(Posterior,nchains=3){
  ...
  function(){
    ...
    curve.pars<-par.means[1:2]
    names(curve.pars)<-c("X.star.log","S.log")#,"a.log","D.log")
    Ab.sdlog<-par.means[3]
    Na.sdlog<-par.means[4]
