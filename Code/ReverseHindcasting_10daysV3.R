...
ll.reverse.hindcasting.gen<-function(ab.values,    #observed antibody test values
  ...
  function(pars){
    ...
    igCurve.current<-igCurve(X.star=exp(curve.pars["curve.pars.X.star.log"]),
                             S=exp(curve.pars["curve.pars.S.log"]),
                             a=exp(curve.pars["curve.pars.a.log"]),
                             D=exp(curve.pars["curve.pars.D.log"]))
    bactLoad.current<-bactLoad(D=exp(curve.pars["curve.pars.D.log"]))
